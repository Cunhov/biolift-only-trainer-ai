...
import { auth, workouts, exercises, users, workoutLimits } from './services/api';
...
const App: React.FC = () => {
  ...
  if (!currentUser) { // Should be guarded by UI but good to check
  ...
  if (!isAdmin) {
    try {
      const check = await users.checkGenerationLimit(currentUser.id);
      if (!check.allowed) {
        alert(check.message);
        return;
      }
      ...
      console.error(err);
      alert('Erro ao verificar limites da conta.');
      return;
    }
  }
...
${userContext}
      ...
      onDone: async (fullContent) => {
        ...
        if (!isAdmin && auth.me()) {
          await users.recordWorkoutGeneration(auth.me().id).catch(console.error);
        }
...
const handleRefineWorkout = async () => {
  ...
  if (!isAdmin) {
    try {
      const check = await workoutLimits.checkEditLimit(selectedWorkout.id, currentUser.id);
      if (!check.allowed) {
        alert(check.message);
      ...
      alert('Erro ao verificar limite de edição.');
      return;
    }
  }
  ...
  try {
    ...
    await poe.chat({
      ...
      onDone: async (fullContent) => {
        ...
        if (!isAdmin && auth.me()) {
          await workoutLimits.recordEdit(selectedWorkout.id).catch(console.error);
          updated.editCount = (updated.editCount || 0) + 1; // Optimistic update
        }
